...
class BasicDeepMatrixFactorization:
    def __init__(self, matrix_factor_dimensions, regularizers):
        self.net = net.DeepMatrixFactorization(matrix_factor_dimensions)
        self.reg = regularizers
        self.loss_dict = {'loss_fid':[],'loss_all':[],'nmae_test':[]}
        for reg in self.reg:
            self.loss_dict[f'loss_{reg.type}'] = []

    def train(self,pic,mu=1,eta=[0],mask_in=None,fid_name=None):
        # loss_all = mu*loss_fid +  eta*loss_reg
        # (Specially, when we choose mu=1, eta=0, We train the model without regularizer)
        ...
        if fid_name is None:
            ...
            raise ValueError(f'Invalid fid_name: {fid_name}')

        loss_reg_list = []
        index_list = []
        j = 0
        for i, reg in enumerate(self.reg):
            if eta[i] is not None:
                index_list.append(j)
                j+=1
                if reg.type == 'hc_reg':
